...
async function fetchContributionSummaryWithClient(
  client: any,
  profileId: string,
  ...
  const [templesRes, editsRes, photosRes] = await Promise.all([
    ...
    client
      .from("temple_edits")
      .select("id, status, points_awarded, temples!inner(id)")
      .eq("profile_id", profileId)
      .in("status", ["approved", "rejected"])
      .is("temples.deleted_at", null),
    client
      .from("temple_photos")
      .select("id, status, points_awarded, temples!inner(id)")
      .eq("profile_id", profileId)
  ...
  const score = [...temples, ...edits, ...photos].reduce(
  ...
  return {
    ...
    rejectedCount:
      rejectedTemples.length + rejectedEdits.length + rejectedPhotos.length,
    score,
  };
}
...
export async function getLeaderboardProfiles() {
  ...
  const { data: profilesData, error: profileError } = await admin
    .from("profiles")
    .select(
      "id, username, full_name, avatar_url, badge, temples_added, edits_made, is_suspended",
    );

  // Exclude suspended profiles
  const profiles = (profilesData || []).filter((p) => !p.is_suspended);
  ...
  const [temples, edits, photos] = await Promise.all([
    admin
      .from("temples")
      .select("created_by, status, points_awarded")
      .is("deleted_at", null)
      .in("status", ["approved", "rejected"]),
    admin
      .from("temple_edits")
      .select("profile_id, status, points_awarded, temples!inner(id)")
      .is("temples.deleted_at", null)
      .in("status", ["approved", "rejected"]),
    admin
      .from("temple_photos")
  ...
  const leaderboard = profiles.map((profile) => {
    const userTemples = templeData.filter(
      (t: any) => t.created_by === profile.id,
    );
    ...
    const approvedTemples = userTemples.filter(
      (t: any) => t.status === "approved",
    ).length;
    const approvedEdits = userEdits.filter(
      (e: any) => e.status === "approved",
    ).length;
    const approvedPhotos = userPhotos.filter(
      (p: any) => p.status === "approved",
    ).length;

    const rejectedTemples = userTemples.filter(
      (t: any) => t.status === "rejected",
    ).length;
    const rejectedEdits = userEdits.filter(
      (e: any) => e.status === "rejected",
    ).length;
    const rejectedPhotos = userPhotos.filter(
      (p: any) => p.status === "rejected",
    ).length;
    const rejectedCount = rejectedTemples + rejectedEdits + rejectedPhotos;

    const score = [...userTemples, ...userEdits, ...userPhotos].reduce(
      (acc, curr) => acc + (curr.points_awarded || 0),
      0,
    );
  ...
  return leaderboard.sort((a, b) => {
    ...
    if (b.temples_added !== a.temples_added)
      return b.temples_added - a.temples_added;
