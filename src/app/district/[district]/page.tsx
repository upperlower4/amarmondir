...
export default async function DistrictPage({ params }: { params: Promise<{ district: string }> }) {
  ...
  const { data: temples } = await supabase.from('temples').select('id, title, slug, division, temple_type, short_bio').eq('district', decoded).eq('status', 'approved').is('deleted_at', null).order('updated_at', { ascending: false });
