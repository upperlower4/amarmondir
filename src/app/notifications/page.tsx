  ...
  const loadData = async (
    ...
    try {
      ...
      const res = await fetch(
        `/api/notifications?filter=${f}&page=${pageNum}`,
        {
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="space-y-4">
          {notifications.map((notif) => (
            ...
            >
              <Card
                className={`p-4 hover:shadow-md transition-shadow relative overflow-hidden ${!notif.is_read ? "border-orange-200 bg-orange-50/50" : "bg-white"}`}
              >
                {!notif.is_read && (
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <h3
                      className={`font-semibold mb-1 ${!notif.is_read ? "text-gray-900" : "text-gray-700"}`}
                    >
                      {notif.title}
                    </h3>
                    <p
                      className={`text-sm ${!notif.is_read ? "text-gray-700" : "text-gray-500"}`}
                    >
                      {notif.body}
                    </p>
                  ...
                  {!notif.is_read && (
                    <Button
                      ...
                      className="h-8 w-8 text-gray-400 hover:text-orange-600 shrink-0"
                    >
        ...
        {hasMore && !loading && (
          <div className="mt-8 text-center">
            <Button
              variant="outline"
              onClick={() => loadData(page + 1, filter, false)}
            >
              আরো দেখুন
            </Button>
          </div>
