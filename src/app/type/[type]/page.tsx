...
export default async function TypePage({ params }: { params: Promise<{ type: string }> }) {
  ...
  const temples = isConfigured
    ? (await supabase.from('temples').select('id, title, slug, division, district, short_bio').eq('temple_type', decoded).eq('status', 'approved').is('deleted_at', null).order('updated_at', { ascending: false })).data
    : [];
