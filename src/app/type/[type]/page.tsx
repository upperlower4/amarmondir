...
export default async function TypePage({
  ...
  return (
    <>
      <main className="flex-1 bg-[#fcfaf7] py-10">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mb-8">
            <Badge className="mb-4">{decoded}</Badge>
            <h1 className="text-4xl font-bold font-serif mb-3">
              {decoded} temples
            </h1>
            <p className="text-gray-600">
              ধরনভিত্তিক SEO browse page for discoverability and internal
              linking.
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {(temples || []).map((item: any) => (
              <Link href={`/temple/${item.slug}`} key={item.id}>
          ...
          {!(temples || []).length && (
            <div className="bg-white border rounded-3xl p-10 text-center text-gray-500 mt-6">
              এই temple type-এ এখনো approved entry নেই।
            </div>
          )}
        </div>
      </main>
