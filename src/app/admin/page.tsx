...
import { Check, X, ExternalLink, Loader2, ShieldCheck, AlertCircle } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleModerate = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      const errorMessage = typeof error === 'string' ? error : error?.message || 'অপারেশন ব্যর্থ হয়েছে';
      toast.error('অপারেশন ব্যর্থ হয়েছে', { description: String(errorMessage).slice(0, 500) });
    } finally {
      setProcessingId(null);
    }
  };

  if (authLoading || (profile && !profile.is_admin && !authLoading)) {
       ...
       return (
         <div className="min-h-screen flex items-center justify-center p-4">
           <Card className="max-w-md w-full text-center p-8 space-y-4">
             ...
             <Button asChild className="bg-orange-500"><Link href="/">হোমপেজে ফিরে যান</Link></Button>
           </Card>
         </div>
       );
     }
                      ...
                      <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-3 bg-gray-50 px-4 py-2 rounded-xl h-fit">
                           ...
                           <div className="text-xs">
                             <p className="font-bold">{temple.profiles?.full_name || temple.profiles?.username}</p>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex gap-2">
                           <Button variant="outline" size="sm" asChild>
                              <Link href={`/temple/${temple.slug}?preview=true`} target="_blank">
                                <ExternalLink className="h-4 w-4 mr-2" /> প্রিভিউ
                              </Link>
                           </Button>
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {processingId === temple.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4 mr-2" />} এপ্রুভ
                          </Button>
                        </div>
                      </div>
                    </CardContent>
